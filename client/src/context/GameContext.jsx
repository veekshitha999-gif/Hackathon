...
export const fallbackDefaultQuizzes = [
  {
    id: 'quiz-1',
...
export function GameProvider({ children }) {
  ...
  const [quizzes, setQuizzes] = useState(fallbackDefaultQuizzes);
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // Fetch initial quiz list via Socket
  ...
  return (
    <GameContext.Provider value={{
      socket,
      isConnected,
      quizzes: (Array.isArray(quizzes) && quizzes.length > 0) ? quizzes : fallbackDefaultQuizzes,
      refreshQuizzes,
      activeView,
      setActiveView,
      editingQuiz,
